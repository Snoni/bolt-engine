	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT) {
			...
			switch(options[curSelected]) {
				...
				case 'Extra Gameplay':
					openSubState(new ExtraSubstate());
...
class ExtraSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'No Icons',
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			case 'Vertical Healthbar':
				daText = "Flips the healthbar vertically and places it to the side.";
			case 'Icons In Corner':
				daText = "Places the icons above the respective characters and removes the healthbar.";
			case 'Disable Gold Notes':
				daText = "Disables x2 score multiplier notes. (not recommended)";
		}
		descText.text = daText;
